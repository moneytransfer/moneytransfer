...
public class TransactionFeeSharing {
							...
							if (_PayOutAgent.next()) {
								...
								if (_TransactionFeeSharing.TransactionFeeSharingId <= 0) {	
									...
									if (!_checkExist.next()) {
										...
										String sUpdateStatement = "UPDATE transactionfeesharing SET Code = ?"	+ " WHERE TransactionFeeSharingId = ?";
										_PreparedStatement = _Connection.prepareStatement(sUpdateStatement);
										_PreparedStatement.setString(1,_code);
										_PreparedStatement.setInt(2, _TransactionFeeSharing.TransactionFeeSharingId);
										...
										_TransactionFeeSharing.setError(_TransactionFeeType +" Fee Sharing already under Code "+_Exitscode +" please edit "+_TransactionFeeType+" Fee Sharing Code "+ _Exitscode +" for new requirements");
										 clear(_TransactionFeeSharing);
									}
	...
	private int _updateTransactionFeeSharing(int TransactionFeeSharingId,int CompanyId, String TransactionFeeType, int PaymentMethodId,
		...
		try{
			...
			_PreparedStatement.setString(9, CreateDate);
			_PreparedStatement.setBoolean(10, IsSpecific);
			_PreparedStatement.setInt(11, TransactionFeeSharingId);
			_PreparedStatement.executeUpdate();
	...
	public static ArrayList<TransactionFeeSharing> getTransactionFeeSharingDetailsByCompany(int CompanyId) {
		...
		try {
...

			{
				if (CompanyId == 0) {
					ResultSet _ResultSet = _MYSQLHelper.GetResultSet(
							"SELECT * FROM transactionfeesharing where IsDeleted=0",
							_Connection);
	...
	public TransactionFeeSharing gettransactionFeeSharing(int TransactionFeeSharingId) {
		...
		try {
			if (_Connection != null) {
				ResultSet _Transaction = _MYSQLHelper.GetResultSet(
						"SELECT * FROM transactionfeesharing where TransactionFeeSharingId='" + TransactionFeeSharingId + "'",
						_Connection);
