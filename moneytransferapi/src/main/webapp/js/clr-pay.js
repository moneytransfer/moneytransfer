...
function Alert(type,msg) {  
    ...
    switch (type) {
        case 1:
            div = "<div id='alert' class='alert alert-success pull-left col-sm-8 alertmain'><a href='#' class='close' data-dismiss='alert'>&times;</a><strong>Success! </strong> " + msg + "</div>";
            break;
        case 2:
            div = "<div id='alert' class='alert alert-danger pull-left  col-sm-8 alertmain'><a href='#' class='close' data-dismiss='alert'>&times;</a><strong>Warning! </strong> " + msg + "</div>";
            break;
    }
    $('.payment').prepend(div);
    setTimeout(function () {
...
function validateinfo()
{
    ...
    var _cardType=$.payform.parseCardType(cardNumber.val());
   ...
   if(owner.val().length < 3){
    	owner.focus();        	
        Alert(2,"Invalid card holder name");
        return false;
    }        
    else if (!isCardValid) {
    	cardNumber.focus();
        Alert(2,"Invalid card number");
        return false;
    } 
	...
	{
	 CVV.focus();
     Alert(2,"Invalid CVV number");
     return false;
	}
    else if (isCvvValid) {
    	if(_cardType=="amex" && CVV.val().length=="4" || _cardType!="amex" && CVV.val().length=="3")
    	{
    		if (month.val()=="") {
    	    	month.focus();
    	        Alert(2,"Please select card expiry month.");
    	        return false;
    	    }
    		if (year.val()=="") {
    	    	year.focus();
    	        Alert(2,"Please select card expiry year.");
    	        return false;
    	       }
    		   var _month=month.val();
    		    var _year=year.val();
    		if(_month< currentMonth &&  _year<= currentYear)
    			{
    			year.focus();
    	        Alert(2,"Your card is expired!.");
    	        return false;
    			}
    		if(v.length == 0)
    		{
    		   // document.getElementById('captcha').innerHTML="You can't leave Captcha Code empty";
    			Alert(2,"You can't leave Captcha empty!.");  		   
    		    return false;    		   
