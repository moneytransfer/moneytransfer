...
(function () {
    ...
    feesController.$inject = ['$scope', '$http', '$localStorage', '$location', '$rootScope', '$anchorScroll', '$timeout', '$window', '$state', '$stateParams', '$translate'];
    function feesController($scope, $http, $localStorage, $location, $rootScope, $anchorScroll, $timeout, $window, $state, $stateParams, $translate) {

        var vm = $scope;
        var IsAdmin = false;
        vm.ManageUsers = [{ PaymentMethod: "", Fees: "", CreatedDate: "", ChargeSendingAmount :""}];
        vm.UserId = 0;
        vm.CompanyId = 0;
        ...
        .success(function (data) {
            ...
            for (var i = 0; i < idata.length; i++) {
                vm.ManageUsers[i].Fees = idata[i].Fees;
                vm.ManageUsers[i].CreatedDate = idata[i].CreatedDate;
                vm.ManageUsers[i].ChargeSendingAmount = idata[i].ChargeSendingAmount;
                vm.getPaymentMethodDetails(idata[i].PaymentMethodId, i);
            }
        ...
        vm.getPaymentMethodDetails = function (PaymentMethodId, index) {
            var formData = JSON.parse(JSON.stringify({ "PaymentMethodId": PaymentMethodId }));
       ...
       .success(function (data) {
           var idata = data;
           vm.ManageUsers[index].PaymentMethod = idata.Title;
